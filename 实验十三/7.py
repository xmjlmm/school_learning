...
if __name__ == "__main__":
    ...
    for ii in range(NUM_FRAMES):
        print("frame: ", ii)
        ret, frame = cap.read()
        if not ret:  # 检查帧是否读取成功
            break

        # 直接使用读取的帧，跳过重映射和缩放
        img = frame  # 直接使用原始帧
        # img = cv2.resize(img, (400,400))  # 如果需要调整大小，可以取消注释这行
        ...
        # 获取所有滑窗及其位置
        all_results = list(sliding_window(Amplitude, theta, patch_size=(Ni, Nj)))
        if all_results:
            indices, patches = zip(*all_results)
        else:
            indices, patches = [], []
