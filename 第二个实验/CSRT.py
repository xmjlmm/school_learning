...
def create_csrt_tracker():
    ...
    创建CSRT跟踪器，使用多种方法尝试
    """
    print("🔧 Trying to create CSRT tracker...")
    ...
    for i, method in enumerate(creation_methods):
        try:
            ...
            if tracker is not None:
                print(f"✅ Method {i+1} successfully created CSRT tracker")
                return tracker
        except Exception as e:
            print(f"❌ Creation method {i+1} failed: {e}")
            continue
    
    raise ValueError("❌ Cannot create CSRT tracker with any method")
    ...
    def initialize_trackers(self, frame, bboxes):
        ...
        for i, bbox in enumerate(bboxes):
            try:
                ...
                if not success:
                    print(f"⚠️ CSRT_{i+1} initialization failed")
                    continue
                
                self.trackers.append(tracker)
                self.last_bboxes.append(bbox_int)
                success_count += 1
                print(f"✅ CSRT_{i+1} initialized successfully with bbox: {bbox_int}")
                
            except Exception as e:
                print(f"❌ CSRT_{i+1} initialization error: {e}")
                continue
        
        print(f"📊 CSRT tracker initialization result: {success_count}/{len(bboxes)} successful")
    ...
    def update(self, frame):
        ...
        for i, tracker in enumerate(self.trackers):
            try:
                ...
                print(f"❌ CSRT_{i+1} update error: {e}")
                success = False
                bbox = self.last_bboxes[i] if i < len(self.last_bboxes) else None
...
def main():
    print("🚀 Single Object Tracking with CSRT Algorithm")
    print("=" * 50)
    ...
    print("4. Press 'q' to end selection")c
    print("=" * 50)
    ...
    print(f"🎯 Starting to track {len(bboxes)} targets with CSRT...")
    ...
    print("✅ CSRT tracker ready for tracking")
    ...
    print("💡 Tips:")
    print("   - Green: Tracking successful")
    print("   - Gray: Tracking lost")
    print("   - CSRT works well with complex backgrounds and scale changes")
    ...
    while True:
        ...
        for i, (success, bbox) in enumerate(zip(success_flags, pred_boxes)):
            if bbox is not None and bbox[2] > 0 and bbox[3] > 0:
                ...
                cv2.rectangle(display_frame, (x, y), (x + w, y + h), color, 2)
                put_text(display_frame, f"CSRT_{i+1}({status_text})", (x, y-10), 0.5, color, 1)
        ...
        cv2.imshow('CSRT Object Tracking', display_frame)
...
def plot_performance_results(evaluator, processing_times):
    ...
    try:
        ...
        plt.ylabel('IOU')
        plt.title('CSRT Tracker - IOU Over Time')
        plt.legend()
        ...
        plt.title('CSRT Tracker - Success Rate Over Time')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        plt.xlabel('Frame Number')
        plt.ylabel('FPS')
        plt.title('CSRT Tracker - Processing Speed')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        plt.title('CSRT Tracker - Center Error')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig('csrt_tracking_performance.png', dpi=300, bbox_inches='tight')
        plt.show()
        print("\n✅ Performance chart saved as 'csrt_tracking_performance.png'")
