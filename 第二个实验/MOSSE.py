...
def create_mosse_tracker():
    ...
    创建MOSSE跟踪器，使用多种方法尝试
    """
    print("🔧 Trying to create MOSSE tracker...")
    
    # 尝试多种创建方法
    creation_methods = [
        # 方法1: 尝试legacy模块 (OpenCV 4.x)
        lambda: cv2.legacy.TrackerMOSSE_create() if hasattr(cv2, 'legacy') and hasattr(cv2.legacy, 'TrackerMOSSE_create') else None,
        
        # 方法2: 尝试直接创建 (OpenCV 3.x/4.x)
        lambda: cv2.TrackerMOSSE_create() if hasattr(cv2, 'TrackerMOSSE_create') else None,
        
        # 方法3: 尝试新API
        lambda: cv2.TrackerMOSSE.create() if hasattr(cv2, 'TrackerMOSSE') else None,
    ]
    ...
    for i, method in enumerate(creation_methods):
        try:
            ...
            if tracker is not None:
                print(f"✅ Method {i+1} successfully created MOSSE tracker")
                return tracker
        except Exception as e:
            print(f"❌ Creation method {i+1} failed: {e}")
            continue
    
    raise ValueError("❌ Cannot create MOSSE tracker with any method")
    ...
    def initialize_trackers(self, frame, bboxes):
        ...
        for i, bbox in enumerate(bboxes):
            try:
                ...
                if not success:
                    print(f"⚠️ MOSSE_{i+1} initialization failed")
                    continue
                
                self.trackers.append(tracker)
                self.last_bboxes.append(bbox_int)
                success_count += 1
                print(f"✅ MOSSE_{i+1} initialized successfully with bbox: {bbox_int}")
                
            except Exception as e:
                print(f"❌ MOSSE_{i+1} initialization error: {e}")
                continue
        
        print(f"📊 MOSSE tracker initialization result: {success_count}/{len(bboxes)} successful")
    ...
    def update(self, frame):
        ...
        for i, tracker in enumerate(self.trackers):
            try:
                ...
                print(f"❌ MOSSE_{i+1} update error: {e}")
                success = False
                bbox = self.last_bboxes[i] if i < len(self.last_bboxes) else None
...
def main():
    print("🚀 Single Object Tracking with MOSSE Algorithm")
    print("=" * 50)
    ...
    # 设置合适的帧延迟 - MOSSE速度很快，可以设置较小的延迟
    frame_delay = 1  # MOSSE速度快，可以设置较小延迟
    ...
    print(f"🎯 Starting to track {len(bboxes)} targets with MOSSE...")
    
    # 初始化MOSSE跟踪器
    tracker = MOSSETracker()
    evaluator = TrackingEvaluator()
    
    print("🔧 Initializing MOSSE tracker...")
    if not tracker.initialize_trackers(frame, bboxes):
        print("❌ MOSSE tracker initialization failed!")
        cap.release()
        return
    
    print("✅ MOSSE tracker ready for tracking")
    ...
    print("💡 Tips:")
    print("   - Yellow: Tracking successful")
    print("   - Gray: Tracking lost")
    print("   - MOSSE is optimized for speed and works best with consistent lighting")
    ...
    while True:
        ...
        for i, (success, bbox) in enumerate(zip(success_flags, pred_boxes)):
            if bbox is not None and bbox[2] > 0 and bbox[3] > 0:
                ...
                cv2.rectangle(display_frame, (x, y), (x + w, y + h), color, 2)
                put_text(display_frame, f"MOSSE_{i+1}({status_text})", (x, y-10), 0.5, color, 1)
        ...
        cv2.imshow('MOSSE Object Tracking', display_frame)
...
def plot_performance_results(evaluator, processing_times):
    ...
    try:
        ...
        plt.ylabel('IOU')
        plt.title('MOSSE Tracker - IOU Over Time')
        plt.legend()
        ...
        plt.title('MOSSE Tracker - Success Rate Over Time')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        plt.xlabel('Frame Number')
        plt.ylabel('FPS')
        plt.title('MOSSE Tracker - Processing Speed')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        plt.title('MOSSE Tracker - Center Error')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig('mosse_tracking_performance.png', dpi=300, bbox_inches='tight')
        plt.show()
        print("\n✅ Performance chart saved as 'mosse_tracking_performance.png'")
