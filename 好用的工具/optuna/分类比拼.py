...
def objective(trial):
    ...
    classifier_name = trial.suggest_categorical("classifier", ["SVC", "RandomForest", "NeuralNetwork", "XGBoost", "DecisionTree"])
    ...
    if classifier_name == "SVC":
        ...
        dt_max_depth = trial.suggest_int("dt_max_depth", 1, 32, log=True)
        dt_min_samples_split = trial.suggest_int("dt_min_samples_split", 2, 20)
        dt_min_samples_leaf = trial.suggest_int("dt_min_samples_leaf", 1, 10)
        dt_max_features = trial.suggest_categorical("dt_max_features", ["sqrt", "log2"])
        classifier_obj = sklearn.tree.DecisionTreeClassifier(
            max_depth=dt_max_depth, min_samples_split=dt_min_samples_split,
            min_samples_leaf=dt_min_samples_leaf, max_features=dt_max_features
        )
