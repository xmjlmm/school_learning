...
# 初始化粒子位置和速度
positions = np.random.rand(population_size, num_cities)  # 位置：随机生成路径
velocities = np.random.rand(population_size, num_cities) * 0.1  # 速度：随机生成速度
...
def bpsop_tsp(cities, population_size=50, max_iter=200, w=0.8, c1=1.5, c2=1.5, alpha=0.5):
    ...
    for iteration in range(max_iter):
        ...
        for i in range(population_size):
            ...
            # 使用sigmoid函数来调整粒子的速度更新幅度
            velocities[i] = velocities[i] * sigmoid(velocities[i])  # Sigmoid控制速度大小
...
plt.title('Convergence Curve of BPSO for TSP with Sigmoid Speed Control')
...
plt.title('Best Path Found by BPSO with Sigmoid Speed Control')
