...
# 数据加载函数 - 从文件夹加载猫狗图片
def load_data_from_folders(cat_folder, dog_folder, max_images_per_class=100, test_split=0.2):
    """
    从猫和狗的文件夹加载图片
    Args:
        cat_folder: 猫图片文件夹路径
...
# 使用ImageDataGenerator的数据加载方式（更节省内存）
def load_data_with_generator(cat_folder, dog_folder, max_images_per_class=5000, validation_split=0.2):
    """
    使用Keras的ImageDataGenerator加载数据，更节省内存[6](@ref)
    """
    # 创建临时目录结构
    temp_dir = 'temp_data'
    ...
    # 由于直接使用原文件夹，这里需要您指定包含猫和狗子文件夹的父目录
    # 例如：data_dir应该包含cats和dogs两个子文件夹
    data_dir = "F:/data/PetImages"  # 请修改为您的实际路径
    ...
    if os.path.exists(data_dir):
        ...
        print(f"数据目录 {data_dir} 不存在，使用随机数据代替")
        return None, None

# AlexNet模型定义
def alex_net(activation_func='relu'):
    """定义AlexNet模型[3,4](@ref)"""
...
def create_callbacks(experiment_name):
    ...
    # 保存最佳模型
    best_model_path = f"weights/best_{experiment_name}.h5"
...
def plot_training_history(history, experiment_name):
    ...
    plt.title(f'{experiment_name} - 准确率')
    plt.xlabel('Epoch')
    plt.ylabel('准确率')
    plt.legend()
    ...
    plt.title(f'{experiment_name} - 损失')
    plt.xlabel('Epoch')
    plt.ylabel('损失')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig(f'visualizations/training_curves_{experiment_name}.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def train_model():
    ...
    # 创建模型
    print("创建AlexNet模型...")
    model = alex_net(activation_func='relu')
    ...
    # 创建回调函数
    callbacks = create_callbacks('alexnet_cat_dog')
    ...
    # 保存最终模型
    final_model_path = "weights/alexnet_cat_dog_final.h5"
    model.save(final_model_path)
    print(f"最终模型已保存: {final_model_path}")
    
    # 绘制训练曲线
    plot_training_history(history, 'AlexNet_Cat_Dog_Classification')
    ...
    # 保存为JSON
    import json
    history_path = "training_logs/training_history.json"
...
def main():
    ...
    try:
        ...
        print("\n训练完成！")
        print("生成的文件:")
        print("- weights/best_alexnet_cat_dog.h5 (最佳模型)")
        print("- weights/alexnet_cat_dog_final.h5 (最终模型)")
        print("- visualizations/training_curves_*.png (训练曲线)")
        print("- training_logs/training_history.json (训练历史)")
