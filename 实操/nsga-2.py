...
def generate_individual():
    ...
    return creator.Individual([(8 * random.random() - 4) for _ in range(2)])
...
def feasible(individual):
    ...
    return x1 > -4 and x2 > -4 and x1 < 4 and x2 < 4 and 1 / x1 + x2 - 4 <= 0

# 修复个体，使其满足变量范围和避免复数
def repair(individual):
    for i in range(len(individual)):
        # 修复个体，使其在规定范围内
        individual[i] = min(max(individual[i], -4), 4)
        # 确保不出现复数
...
toolbox.register("mutate", tools.mutPolynomialBounded, low=-4, up=4, eta=0.1, indpb=0.2)
# 选择操作，使用非支配排序NSGA-II
...
def mutate_and_repair(individual):
    tools.mutPolynomialBounded(individual, low=-4, up=4, eta=0.1, indpb=0.2)
    repair(individual)
    return individual,
...
plt.subplot(1, 2, 2)
plt.plot(best_f2, label="f2: (x1-1)^2 + (x2+1)^2")
plt.xlabel("Generations")
plt.ylabel("Best f2")
