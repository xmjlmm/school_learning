...
class IMDBProcessor:
    def __init__(self, config):
        ...
        self.embed_matrix  = [np.zeros(config.embed_dim), np.random.normal(size=config.embed_dim)] # <pad>的零向量 <unk>的随机初始化向量 
    ...
    def text_to_indices(self, text):
        ...
        indices = [self.word2idx.get(token, 1) for token in tokens[:self.config.max_seq_len]] 
...
class RNNModel(nn.Module):
    def __init__(self, vocab_size,embed_matrix):
        super().__init__()
        self.embed  = nn.Embedding.from_pretrained(embed_matrix,  padding_idx=0)
        self.rnn  = nn.RNN(Config.embed_dim,  Config.hidden_size,  Config.num_layers,  batch_first=True)
        self.fc  = nn.Linear(Config.hidden_size,  1)
        self.sigmoid  = nn.Sigmoid()
        
    def forward(self, x):
        x = self.embed(x)              # (batch_size, seq_length) => (batch_size, seq_length, embed_dim) 
        outputs, h_n = self.rnn(x)           # outputs:最后一层每个时间步的隐藏状态，形状为 (batch_size, seq_length, hidden_size))
                                             # h_n: 所有层最后一个时间步的隐藏状态 (num_layers, batch_size, hidden_size)
        return self.sigmoid(self.fc(h_n[-1]))   # 取最后一层隐藏状态 
 
class LSTMModel(nn.Module):
    def __init__(self, vocab_size,embed_matrix):
        super().__init__()
        self.embed  = nn.Embedding.from_pretrained(embed_matrix,  padding_idx=0)
        self.lstm  = nn.LSTM(Config.embed_dim,  Config.hidden_size,  Config.num_layers,  batch_first=True)
        self.fc  = nn.Linear(Config.hidden_size,  1)
        self.sigmoid  = nn.Sigmoid()
        
    def forward(self, x):
        x = self.embed(x) 
        _, (h_n, _) = self.lstm(x)     # LSTM返回隐藏状态和细胞状态 
...
class GRUModel(nn.Module):
    def __init__(self, vocab_size,embed_matrix):
        super().__init__()
        self.embed  = nn.Embedding.from_pretrained(embed_matrix,  padding_idx=0)
        self.gru  = nn.GRU(Config.embed_dim,  Config.hidden_size,  Config.num_layers,  batch_first=True)
        self.fc  = nn.Linear(Config.hidden_size,  1)
        self.sigmoid  = nn.Sigmoid()
        
    def forward(self, x):
        x = self.embed(x) 
        _, h_n = self.gru(x)           # GRU只有隐藏状态 
        return self.sigmoid(self.fc(h_n[-1])) 
 
# 统一训练函数 
def train_and_evaluate(model, train_loader, val_loader, test_loader, config, device):  
    ...
    for epoch in range(config.epochs):   
        ...
        history.log(epoch,   
            ...
            # 绘制可视化曲线  
        with canvas:  
            canvas.draw_plot([history["train_loss"],  history["val_loss"]], labels=["Train Loss", "Val Loss"])  
            canvas.draw_plot([history["train_acc"],  history["val_acc"]], labels=["Train Acc", "Val Acc"])  
...
# 初始化模型 
models = {
    "RNN": RNNModel(len(processor.embed_matrix),  processor.embed_matrix).to(device), 
    "LSTM": LSTMModel(len(processor.embed_matrix),  processor.embed_matrix).to(device), 
    "GRU": GRUModel(len(processor.embed_matrix),  processor.embed_matrix).to(device) 
}
...
for name, model in models.items():   
    print(f"\n=== Training {name} Model ===")  
    results[name] = train_and_evaluate(model, train_loader, val_loader, test_loader, config, device)  
...
for model_name in models.keys():   
    plt.plot(results[model_name]["train_loss"],  label=f"{model_name} Train Loss")  
    plt.plot(results[model_name]["val_loss"],  linestyle="--", label=f"{model_name} Val Loss")  
plt.legend()   
