...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    # 绘制预测结果散点图并记录到wandb
    # data = [[x, y] for (x, y) in zip(y_test_clean, y_pred_clean)]
    # table = wandb.Table(data=data, columns=["Actual", "Predicted"])
    # scatter_plot = wandb.plot.scatter(table, "Actual", "Predicted", title="Actual vs Predicted")
    # wandb.log({"Actual vs Predicted": scatter_plot})
