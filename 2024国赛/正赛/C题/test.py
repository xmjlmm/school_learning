...
# min_area_per_crops = np.zeros(41)
...
z21 = z_23[:, :, 0]
z21 = pd.DataFrame(z21)
# z21.to_csv('z21.csv', index=False)
...
def get_is_select(x):
    is_select = np.zeros((41, 54, 3, 7), dtype=int)
    for i in range(41):
        for j in range(54):
            for k in range(3):
                for l in range(7):
...
for l in range(7):
    for k in range(3):
        for i in range(41):
            pps = ps[i][k]
            cost_term = lpSum(cc[i][j][k] * x[i][j][k][l] for j in range(54))
            production_sum = lpSum(y[i][j][k][l] * m[i][j][k] for j in range(54))
...
#         for k in range(3):
#             for l in range(7):
#                 model += z[i][j][k][l] <= zzz[i][j][k][l]
for i in range(41):
    for j in range(54):
        for k in range(3):
            for l in range(7):
                # model += x[i][j][k][l] >= 0.3 * zzz[i][j][k][l]
...
temp = 0
for j in range(54):
    temp += sum(z_23[i][j][0] for i in must_select_0)
print("temp", temp)
...
for j in range(26):
    # model += lpSum(z_23[i][j][k] + z[i][j][k][0] + z[i][j][k][1]  >= 1 for k in range(3) for i in must_select_0)
    for l in range(5):
        model += lpSum(z[i][j][k][ll] for k in range(3) for i in must_select_0 for ll in range(l, l + 3)) >= 1
# 双季作物
for i in must_select_12:
    for j in range(54):
        # model += lpSum(z_23[i][j][1] + z_23[i][j][2] + z[i][j][1][0]  >= 1)
        # model += lpSum(z_23[i][j][2] + z[i][j][1][0] + z[i][j][2][0]  >= 1)
        for l in range(5):
            for k in range(1, 2):
                model += z[i][j][k][l] + z[i][j][k + 1][l] + z[i][j][k][l + 1] >= 1
                model += z[i][j][k + 1][l] + z[i][j][k][l + 1] + z[i][j][k + 1][l + 1] >= 1
...
# sum_num = sum_num / num_all

# # 将 re 设为 PuLP 表达式，并通过累乘操作
# re = 1  # 重新初始化 re
# for ii in range(len(sum_num)):
#     re *= sum_num[ii]

# # 将 re 转换为 PuLP 支持的表达式，并加到模型中
# model += LpAffineExpression(re) >= 0.0000002
        ...
        for i in range(41):
            for j in range(54):
                if value(x[i][j][k][l]) > 0:
                    print(
                        f"第{l + 1}年，第{i + 1}种蔬菜在第{j + 1}块地的第{k + 1}种大棚种植面积为{value(y[i][j][k][l])}亩")
# # 创建一个 ExcelWriter 对象
...
#
#         # 将 re 数组转换为 DataFrame 并保存到 Excel 文件的不同工作表中
#         df = pd.DataFrame(re)
#         df.to_excel(writer, sheet_name=f'{yy}', index=False, header=False)
#         yy += 1

# print("最大七年总利润:", value(model.objective)/10000, "万元")
print("最大七年总利润:", value(model.objective) / 10000, "万元")
if model.status == 1:
    print("Optimal")
elif model.status == 0:
...
# 打印最大七年总利润
print("最大七年总利润:", np.sum(re) / 10000, "万元")
