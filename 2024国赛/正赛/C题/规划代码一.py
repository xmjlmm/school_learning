...
h = LpVariable.dicts("h", (range(41), range(54), range(3), range(7)), lowBound=0, cat="Continuous")
...
# # # 目标2
# model += lpSum(y[i][j][k][l] * m[i, j, k] * p[j, i, k] - cc[i, j, k] * x[i][j][k][l] + (
#             m[i, j, k] * x[i][j][k][l] - h[i][j][k][l] * m[i, j, k]) * p[j, i, k] * 0.5 for i in range(41) for j in
#                range(54) for k in range(3) for l in range(7))

# 辅助变量   确保取max(ps,x*m)
# for l in range(7):
#     for i in range(41):
#         for k in range(3):
#             model += lpSum(h[i][j][k][l] * m[i][j][k] for j in range(54)) >= ps[i, k]
#             model += lpSum(h[i][j][k][l] for j in range(54)) >= lpSum(x[i][j][k][l] for j in range(54))
...
#     for k in range(3):
#         for i in range(41):
#             s = np.zeros(6642, dtype=float)
#             for j in range(54):
#                 if x[i][j][k][l] >= 0.3:
#                     s[j] = x[i][j][k][l].varValue
#             for c in s:
#                 print(c)
#             s = s[s > 0]
#             # print(s)
...
#                 chengji *= s[m]
#             model += LpAffineExpression(chengji) >= 1
