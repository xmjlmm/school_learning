...
df = pd.read_excel("F:\\数模\\国赛\\国赛2023\\C题\\附件211.xlsx")
...
c = df['销售单价(元/千克)'].iloc[:n].values  # 确保长度一致
d = df['销量(千克)'].iloc[:n].values  # 确保长度一致
...
def evaluate(individual):
    ...
    # 确保 y 与 beta_0 和 beta_1 的长度一致
    assert len(y) == len(beta_0) == len(beta_1)
