...
K = 0.95  # 温度衰减参数
final_temp = 1  # 终止温度
L_values = [100, 200, 500, 1000]  # 不同的每轮迭代次数
...
for L in L_values:
    ...
    results.append((L, best_distance, best_path, distances))

# 绘制不同L值的收敛曲线
plt.figure(figsize=(10, 6))
for L, best_distance, best_path, distances in results:
    plt.plot(distances, label=f'L = {L}')
plt.xlabel('Iteration')
plt.ylabel('Best Distance')
plt.title('Convergence Curve for Different Iteration Counts (L)')
plt.legend()
plt.show()

# 打印最优解和相应的每轮迭代次数L
for L, best_distance, best_path, distances in results:
    print(f"L = {L}, Best Distance: {best_distance}")
