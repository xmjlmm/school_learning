...
L = 100  # 每轮迭代次数
K = 0.95  # 温度衰减参数
initial_temp = 2000  # 初始温度
final_temps = [10, 50, 100, 150, 200]  # 不同的终止温度
...
for final_temp in final_temps:
    ...
    results.append((final_temp, best_distance, best_path, distances))

# 绘制不同终止温度的收敛曲线
plt.figure(figsize=(10, 6))
for final_temp, best_distance, best_path, distances in results:
    plt.plot(distances, label=f'Final Temp = {final_temp}')
plt.xlabel('Iteration')
plt.ylabel('Best Distance')
plt.title('Convergence Curve for Different Final Temperatures')
plt.legend()
plt.show()

# 打印最优解和相应的终止温度
for final_temp, best_distance, best_path, distances in results:
    print(f"Final Temp: {final_temp}, Best Distance: {best_distance}")
