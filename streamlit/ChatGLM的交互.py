    ...
    with st.container():
        if len(history) > 0:
            if len(history) > MAX_BOXES:
                history = history[-MAX_TURNS:]
            for i, (query, response) in enumerate(history):
                message(query, avatar_style="big-smile", key=f"{i}_user")
                message(response, avatar_style="bottts", key=str(i))

        message(input_text, avatar_style="big-smile", key=f"{len(history)}_user")
        st.write("AI正在回复:")
        with st.empty():
            for response, new_history in model.stream_chat(tokenizer, input_text, history, max_length=max_length, top_p=top_p, temperature=temperature):
