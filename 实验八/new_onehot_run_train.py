...
model = tf.keras.Sequential([
    ...
    SimpleRNN(16, kernel_regularizer=l2(0.001)),  # 增加记忆体数量并添加L2正则化
...
def check_model_compatibility():
    ...
    if os.path.exists(checkpoint_save_path):
        try:
            ...
            # 尝试加载权重但不应用（用于检查兼容性）
            model.load_weights(checkpoint_save_path, skip_mismatch=False)
            print("-------------权重文件兼容，加载已有模型-----------------")
            return True
